...
public class PetOwnerRegistration extends JFrame {
    ...
    // --- ΕΛΕΓΧΟΣ ΚΑΙ ΣΥΛΛΟΓΗ ΔΕΔΟΜΕΝΩΝ ΜΕ ΠΕΡΙΟΡΙΣΜΟΥΣ ---
    private String compilePetData() {
        ...
        // 1. [TC_NEG_01] Έλεγχος Ηλικίας Κατοικιδίου
        if (!ageStr.isEmpty()) {
            try {
                int age = Integer.parseInt(ageStr);
                if (age < 0 || age > 30) {
                    JOptionPane.showMessageDialog(this, "Η ηλικία του ζώου πρέπει να είναι μεταξύ 0 και 30 ετών.", "Σφάλμα Ελέγχου Στοιχείων", JOptionPane.ERROR_MESSAGE);
                    return null; // Επιστρέφει null για να σταματήσει η ροή
                }
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "Η ηλικία πρέπει να είναι αποκλειστικά αριθμός.", "Σφάλμα Ελέγχου Στοιχείων", JOptionPane.ERROR_MESSAGE);
                return null;
            }
        }
        String age = ageStr.isEmpty() ? "Άγνωστη ηλικία" : ageStr;

        // 2. [TC_NEG_02] Έλεγχος Αντικρουόμενων Επιλογών (Παράδειγμα: Λογικό Σφάλμα)
        if (chkYard.isSelected() && chkIndoor.isSelected()) {
            JOptionPane.showMessageDialog(this, "Έλεγχος Επιλογών: Δεν μπορείτε να επιλέξετε ταυτόχρονα 'Να μένει μόνο σε εσωτερικό χώρο' και 'Απαιτείται αυλή'.", "Μήνυμα Error", JOptionPane.ERROR_MESSAGE);
            return null;
        }
    ...
    private void handleAddMorePet() {
        ...
        String petData = compilePetData();
        if (petData == null) return; // Αν αποτύχει ο έλεγχος στοιχείων (validation), σταματάει η προσθήκη
    ...
    // --- ΟΛΟΚΛΗΡΩΣΗ ΡΟΗΣ ΜΕ ΕΛΕΓΧΟΥΣ ΣΦΑΛΜΑΤΩΝ ---
    private void handleFinalization() {
        ...
        // Έλεγχος Υποχρεωτικού Τηλεφώνου Ανάγκης
        ...
        // Έλεγχος εγκυρότητας τηλεφώνου (Μόνο ψηφία και μήκος π.χ. 10 ψηφία)
        if (!emergencyPhone.matches("\\d{10}")) {
            JOptionPane.showMessageDialog(this, "Το τηλέφωνο ανάγκης πρέπει να αποτελείται από ακριβώς 10 ψηφία.", "Σφάλμα Ελέγχου Στοιχείων", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Αν υπάρχει δεδομένο στη φόρμα που δεν έχει πατηθεί το "Προσθήκη Επιπλέον Ζώου", το ελέγχουμε και το βάζουμε τώρα
        if (!txtName.getText().trim().isEmpty()) {
            String petData = compilePetData();
            if (petData == null) return; // Διακοπή αν αποτύχει το validation
            petsList.add(petData);
        }
        ...
        if (user != null) {
            ...
            JOptionPane.showMessageDialog(this, "Τα στοιχεία των κατοικιδίων αποθηκεύτηκαν επιτυχώς!");
            this.dispose();

            if (isDualRole) {
                // Ροή 3: Αυτόματη μετάβαση στο επόμενο προφίλ (Host)
                new HostRegistration(userEmail, true).setVisible(true);
            } else {
                // Ροή 1: Μετάβαση στην προεπισκόπηση
